...
4. In real mode, create the FD-owned `daens` namespace and the `dae0`/`dae0peer` veth through rtnetlink. The process stays in the host namespace; only synchronous socket, link, and attachment operations enter `daens` through scoped `setns` calls.
